...
merged_df = pd.merge(coffee_quarter_df, pop_agg_quarter_df, on=merge_keys)
merged_df = pd.merge(merged_df, sales_quarter_df, on=merge_keys)
...
if selected_dong == "전체":
    ...
    if not merged_df.empty:
        ...
        with col2:
            st.subheader("유동인구 vs 매출액")
            fig = px.scatter(merged_df, x="총_유동인구_수", y="당월_매출_금액", hover_name="행정동_코드_명", size='당월_매출_금액', color='당월_매출_금액',
                             labels={"총_유동인구_수": "총 유동인구 수", "당월_매출_금액": "당월 매출 금액 (원)"})
            st.plotly_chart(fig, use_container_width=True)
        ...
        with col3:
            ...
            fig = px.bar(df_sorted, x="행정동_코드_명", y="점포_수", title="점포 수 상위 15")
            st.plotly_chart(fig, use_container_width=True)
        with col4:
            st.subheader("매출액 상위 15개 지역")
            df_sorted = merged_df.sort_values(by="당월_매출_금액", ascending=False).head(15)
            fig = px.bar(df_sorted, x="행정동_코드_명", y="당월_매출_금액", title="매출액 상위 15")
            st.plotly_chart(fig, use_container_width=True)
    ...
    sales_per_trans = dong_data['당월_매출_금액'] / dong_data['당월_매출_건수'] if dong_data['당월_매출_건수'] > 0 else 0
    col4.metric("💳 건당 매출액", f"{sales_per_trans:,.0f} 원")
    st.divider()
